...
lz:= cos(DegToRad(spin+90));
  ...
  //  Algoritmo deteccion de colisiones
    if (tempX < deathzone[0]) or (tempX > deathzone[1]) then
      positionX:=tempX;
    if (tempX > deathzone[0]) or (tempX < deathzone[1]) then
      if (tempZ < deathzone[2]) or (tempZ > deathzone[3]) then
        positionX:=tempX;
    if (tempZ < deathzone[2]) or (tempZ > deathzone[3]) then
      positionZ:=tempZ;
    if (positionZ > deathzone[2]) or (positionZ < deathzone[3]) then
      if (positionX < deathzone[0]) or (positionX > deathzone[1]) then
        positionZ:=tempZ;
  // Fin deteccion
...
begin
  ...
  for i := 0 to 1 do
  begin
    deathzone[i] :=   (i-0.5)*4.4;
    deathzone[i+2] := (i-0.5)*4.4;
  end;
end;
